...
export default async function CandidateSettingsPage() {
  ...
  const initialData = {
    name: dbUser.name || "",
    bio: dbUser.candidate.bio || "",
    resumeUrl: dbUser.candidate.resumeUrl || "",
    skills: dbUser.candidate.skills || [],
  };
  ...
  return (
    <div className="container mx-auto px-4 xl:px-8 py-8 md:py-10 max-w-7xl">
      ...
      <nav className="flex items-center gap-2 text-xs font-medium text-zinc-500 mb-6 px-1">
        <Link href="/candidate-dashboard" className="hover:text-zinc-900 dark:hover:text-zinc-300 transition-colors flex items-center gap-1">
      ...
      <div className="mb-8 px-1">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
          Customize your profile, registered skills, resume, and manage your account credentials.
        </p>
      </div>

      {/* Form Component */}
      <CandidateSettingsForm initialData={initialData} />
    </div>
  );
}
