...
import { updateRecruiterProfile, changeUserPassword } from "@/lib/actions/settings";

interface RecruiterSettingsFormProps {
  initialData: {
    name: string;
    companyName: string;
...
export default function RecruiterSettingsForm({ initialData }: RecruiterSettingsFormProps) {
  ...
  const handleProfileSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransitionProfile(async () => {
      const res = await updateRecruiterProfile(formData);
      if (res?.error) {
      ...
      <div className="col-span-3 p-6 sm:p-10">
        ...
        {activeTab === "profile" && (
          <form onSubmit={handleProfileSubmit} className="space-y-6 max-w-2xl animate-in fade-in duration-250">
            <div>
              <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-50 mb-1">Company Profile</h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">Update your company details and recruiter profile settings.</p>
            </div>
            ...
            {profileSuccess && (
              <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl flex items-start gap-3 text-emerald-700 dark:text-emerald-400 animate-in fade-in duration-200">
                ...
                <div>
                  ...
                  <p className="text-xs opacity-90 mt-0.5">Your updated company and recruiter profile details are now active.</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-5">
              ...
              {/* Company Name */}
              <div className="space-y-2">
                <label htmlFor="companyName" className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                  Company Name
                </label>
                <div className="relative">
                  <Building2 className="absolute left-3.5 top-3.5 w-4 h-4 text-zinc-400" />
                  <input
                    id="companyName"
                    name="companyName"
                    type="text"
              ...
              {/* Company Website */}
              <div className="space-y-2">
                <label htmlFor="companyWebsite" className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                  Company Website URL
                </label>
                <div className="relative">
              ...
              {/* About Company */}
              <div className="space-y-2">
                <label htmlFor="aboutCompany" className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                  About Company
                </label>
                <div className="relative">
                  ...
                  <textarea
                    ...
                    disabled={isPendingProfile}
                    placeholder="Describe your company, work culture, and environment..."
